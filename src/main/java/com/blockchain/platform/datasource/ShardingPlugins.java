package com.blockchain.platform.datasource;
 ...
 * @author DENGLONG
 * @version 1.0
 * @create 2019-05-18 5:22 PM
 **/
@Setter
@Accessors(chain = true)
@Intercepts({ @Signature(type = StatementHandler.class, method = "prepare", args = {Connection.class, Integer.class}) })
public class ShardingPlugins extends AbstractSqlParserHandler implements Interceptor{

    /**
     * 实际执行sql
     */
    private static final String DB_BOUND_SQL = "delegate.boundSql.sql";

    private static final String DB_BOUND = "delegate.boundSql";
     ...
     * sql 执行参数
     */
    private static final String DB_BOUND_PARAMETER = "delegate.boundSql.parameterObject";

    /**
     * statement对象
     */
    private static final String DB_BOUND_STATEMENT = "delegate.mappedStatement";
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        MetaObject meta = MetaObject.forObject(handler, DEFAULT_OBJECT_FACTORY, DEFAULT_OBJECT_WRAPPER_FACTORY, DEFAULT_OBJECT_REFLECTOR_FACTORY);
        BoundSql boundSql = (BoundSql) meta.getValue( DB_BOUND);
        // sql执行参数
        Object params = meta.getValue( DB_BOUND_PARAMETER);
        // 当前Mapper
        MappedStatement mapped = (MappedStatement) meta.getValue( DB_BOUND_STATEMENT);
        // 类名
        String className = StrUtil.subBefore( mapped.getId(), StrUtil.DOT,  Boolean.TRUE);
        // 当前对象
        Class<?> clazz = Class.forName( className);
        // 是否分表
        if( ObjectUtil.isNotEmpty( params) && clazz.isAnnotationPresent( Sharding.class)) {
            String sql = (String) meta.getValue( DB_BOUND_SQL);
            if( StrUtil.isNotEmpty( sql)){
                // 验证是否存储 分表字段
                Sharding sharding = clazz.getAnnotation( Sharding.class);
                // 分表值
                Object value = BeanUtil.getFieldValue( params, sharding.field());
                if( ObjectUtil.isNotEmpty( value)){
                    // 是否分表
                    boolean ds = false;
                    for ( int idx = 0;idx < sharding.table().length; idx ++) {
                        String tb = sharding.table()[idx];
                        if( StrUtil.containsAny( sql, tb)) {
                            ds = true;
                            String table = StrUtil.toString( value);
//                            if ( StrUtil.containsAny( table, StrUtil.DASHED)) {
//                                table = StrUtil.replace( table, StrUtil.DASHED, StrUtil.UNDERLINE);
//                            }
                            sql = StrUtil.replace( sql,
                                                    tb,
                                                    StrUtil.addPrefixIfNot( StrUtil.concat(Boolean.FALSE , table.toLowerCase() , StrUtil.UNDERLINE, tb), TABLE_PREFIX),
                                                    Boolean.TRUE);
                        }
                    }
                    if( ds) {
                        meta.setValue(DB_BOUND_SQL,  sql);
                    }
